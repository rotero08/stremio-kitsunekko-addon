...
def analyze_text(line, season, episode):
    ...
    if season_match:
        ...
        if episode_match:
            ...
            if numbers_in_line(cleaned_line) == 1:
                episode_number = re.search(r'\d+', cleaned_line).group()
                results.append(f"CleanedLine '{line.strip()}': No season specified, assuming season 1; episode is {episode_number} (remaining number).") if episode_number == episode else ''
            else: 
...
def main(anime_name, season, episode):
    ...
    for file in subtitle_files:
        results.append(analyze_text(file, season, episode))
